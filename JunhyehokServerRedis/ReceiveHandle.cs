...
namespace JunhyehokServerRedis
{
    class ReceiveHandle
    {
        ...
        public ReceiveHandle(Socket authSocket, RedisHandlerForFE redisHandler, string mmfNombre)
        {
            auth = authSocket;
            redis = redisHandler;
            mmfName = mmfNombre;
        ...
        public Packet ResponseInitialize(Packet recvPacket)
        {
            ...
            if (authorized)
            {
                ...
                auth.SendBytes(backendReqPacket);
            }
            else
            {
                returnData = null;
        ...
        public Packet ResponseServerStop(Packet recvPacket)
        {
            if (IsAgent())
            {
                ...
                try
                {
                    auth.Shutdown(SocketShutdown.Both);
                    auth.Close();
                }
        ...
        public Packet GetResponse()
        {
            ...
            try
            {
                if (client.So.RemoteEndPoint.ToString() == auth.RemoteEndPoint.ToString())
                    isAuth = true;
            }
            catch (Exception) { isAuth = false; }
            
            if (!IsAgent() && !isAuth && !HasInitialized())
